...
#include "node_atc18_aor221d_z.h"

//register to node factory
std::string Node_ATC18_AOR221D_Z::node_factory_dummy = Node_factory<Node>::register_create_function("ATC18_AOR221D_Z",Node_ATC18_AOR221D_Z::create_function);
std::string Node_ATC18_AOR221D_Z::node_factory_dummy2 = Node_factory<Node>::register_output("ATC18_AOR221D","Z");

Node_ATC18_AOR221D_Z::Node_ATC18_AOR221D_Z(const char *name) : Node(name, 5){
...
Node_ATC18_AOR221D_Z::~Node_ATC18_AOR221D_Z()
{
}

std::string Node_ATC18_AOR221D_Z::get_input_name(int i)
{
    switch(i){
        case(0): return("A");
        case(1): return("B1");
...
int Node_ATC18_AOR221D_Z::get_value(int run)
{
//define some helpers, makes it easier to write function
    ...
    if (last_run != run){
        //need to recalc
        last_run_value = (A || (B1 && B2) || (C1 && C2));
        last_run = run;
    }
    return last_run_value;
}

std::string Node_ATC18_AOR221D_Z::get_formula()
{
//define some helpers, makes it easier to write function
    ...
    if (is_safe){
        ...
        return std::string("(" + formula_A + " || (" + formula_B1 + " && " + formula_B2 + ") || (" + formula_C1 + " && " + formula_C2 + "))");
    }
}
