...
#include "node_atc18_aoi31d_zn.h"

//register to node factory
std::string Node_ATC18_AOI31D_ZN::node_factory_dummy = Node_factory<Node>::register_create_function("ATC18_AOI31D_ZN",Node_ATC18_AOI31D_ZN::create_function);
std::string Node_ATC18_AOI31D_ZN::node_factory_dummy2 = Node_factory<Node>::register_output("ATC18_AOI31D","ZN");

Node_ATC18_AOI31D_ZN::Node_ATC18_AOI31D_ZN(const char *name) : Node(name, 4){
...
Node_ATC18_AOI31D_ZN::~Node_ATC18_AOI31D_ZN()
{
}

std::string Node_ATC18_AOI31D_ZN::get_input_name(int i)
{
    switch(i){
        case(0): return("A");
        case(1): return("B3");
...
int Node_ATC18_AOI31D_ZN::get_value(int run)
{
//define some helpers, makes it easier to write function
    ...
    if (last_run != run){
        //need to recalc
        last_run_value = !((B1 && B2 && B3) || A);
        last_run = run;
    }
    return last_run_value;
}

std::string Node_ATC18_AOI31D_ZN::get_formula()
{
//define some helpers, makes it easier to write function
    ...
    if (is_safe){
        ...
        return std::string("!((" + formula_B1 + " && " + formula_B2 + " && " + formula_B3 + ") || " + formula_A + ")");
    }
}
