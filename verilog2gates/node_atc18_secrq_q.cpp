...
#include "node_atc18_secrq_q.h"

//register to node factory
std::string Node_ATC18_SECRQ_Q::node_factory_dummy = Node_factory<Node>::register_create_function("ATC18_SECRQ_Q",Node_ATC18_SECRQ_Q::create_function);
std::string Node_ATC18_SECRQ_Q::node_factory_dummy2 = Node_factory<Node>::register_output("ATC18_SECRQ","Q");

Node_ATC18_SECRQ_Q::Node_ATC18_SECRQ_Q(const char *name) : Node(name, 4){
...
Node_ATC18_SECRQ_Q::~Node_ATC18_SECRQ_Q()
{
}

std::string Node_ATC18_SECRQ_Q::get_input_name(int i)
{
    switch(i){
        case(0): return("CP");
        case(1): return("ENN");
...
int Node_ATC18_SECRQ_Q::evaluate()
{
//define some helpers, makes it easier to write function
    ...
    int run = last_eval++;
    last_run_value = (1?(CDN?(SC?(ENN?SD:SD):(ENN?Q:D)):0):1);
    return last_run_value;
}

Node *Node_ATC18_SECRQ_Q::get_ff_input_node()
{
...
std::string Node_ATC18_SECRQ_Q::get_formula()
{
