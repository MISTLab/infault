...
#define RADIATOR_QUIET 0
#define RADIATOR_QUIET2 0
...
int Radiator::do_seu_search(Graph *g, Node *node, int ffs_done)
{
    if (node == NULL){
    ...
    if (node->get_child_ff_nodes().size() <= SPLITSIZE){
        ...
        for(int t=0; t<node_a2->get_input_node_count(); t++){
            ...
            if (chx->get_child_ff_nodes().size() > SPLITSIZE){
                ...
                ///string inputname = chx->get_name() + string("__DUMMY");
...
{   
    ...
    //get "input" nodes
    vector <Node*> source_vector2 = target->get_child_input_nodes();
    ...
    int groupid[source_vector.size() + source_vector2.size()];
    int last = 0;
    int group_counter = 0;
...
//    return 1;
    ...
    //ok now add all input nodes, add them here because they all get a 
    //unique group id -> injection in all inputs -> simulate all possible input combos
    for(int i=0; i<source_vector2.size(); i++){
        last++;
        source_vector.push_back(source_vector2.at(i));
        groupid[source_vector.size()-1] = (1<<last);
